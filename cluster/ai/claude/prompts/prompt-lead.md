...
6. Set a `model::` label. **Default to `model::opus`**: most planned work needs judgment, multi-file reasoning, or non-trivial design. Use `model::sonnet` only for trivial mechanical work: single-file rename, formatting fixes, dependency bumps with no behaviour change, doc-only tweaks.
